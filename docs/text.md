...
| `format` | `MB`/`KB`/`GB`/`size` on a file-size value. A **date FIELD** value throws - chain it: `<ref:datetimeeval in="@dateVar" addHours="0" format="..."/>` (a time-granularity add is required). A record date **property** (`out="createdonlocal"`) formats directly. |
...
| `onEmpty` | Value to emit when the resolved value is empty **or null** - the primary guard for a field that may be missing. Runs before `format`/`left`/`right`/`case`/`encode`. |
...
- **`encode`** — `html`, `htmlwithbreaks` (html plus newline -> `<br />`), `xml` (escapes only `&` `<` `>`; quotes stay raw), or `json`. Live-verified.
